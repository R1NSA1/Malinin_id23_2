...
class Planet:
    def __init__(self, name, distance_from_sun, speed, density):
        self.name = name
        self.distance_from_sun = distance_from_sun  # расстояние от солнца в пикселях
        self.speed = speed  # скорость вращения (градусы за обновление)
        self.density = density  # плотность планеты
        self.angle = random.uniform(0, 360)  # начальный угол на орбите
...
class SolarSystem(QWidget):
    def __init__(self):
        ...
        self.timer.start(15)  # Обновляем каждые 50 мс
